...
haveEnoughStorage = (prices, reserved) => prices.every(price => getResourceMax(price.name) >= price.val / spendResourceFullRatio + (reserved[price.name] || 0))
...
//todo use ticksPerLoop and game.getResourcePerTick(res, true)
isResourceFull = res => getResourceOwned(res) / getResourceMax(res) > craftResourceFullRatio;
...
makeCraft = (craft, amountNeeded, reserved) => {
    ...
    if (canAfford(totalPrices, reserved)) {
        ...
        while (maxClicks > 0 && timesToCraft > 0) {
            var targetButton;
            var targetCraftTimes;
            findCraftButtons(craft).each((idx, button) => {
...
//todo sort queue by number bought//price ratio
...
craftResourceFullRatio = .993
spendResourceFullRatio = .99
loopHandle = 0;
